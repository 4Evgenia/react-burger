...
const FeedItemDetails: FC<FeedItemDetailsProps> = ({ feedItem }) => {
    ...
    return (<div className={styles.orderContainer}>
        <section className="text text_type_digits-default">#{feedItem.number}</section>
        <section className={`${styles.startAllign} text text_type_main-medium mt-10`}>
            {feedItem.title}
        </section>
        <section className={`${styles.startAllign} mt-3`}>
            <Status status={feedItem.status} />
        </section>
        <section className={`${styles.startAllign} text text_type_main-medium mt-10 mb-6`}>
            Состав:
        </section>
        <section className={styles.scroll}>
            {feedItem.ingredients.map(ing => {
                return (
                    <div key={ing._id} className={`${styles.ingredientContainer} mt-4 mr-6`}>
                        <div className={styles.group}>
                            <div className={`${styles.imageContainer} mr-4`}>
                                <img src={ing.image_mobile} alt={ing.name} />
                            </div>
                            <div className="text text_type_main-default">{ing.name}</div>
                        </div>
                        <div className={styles.group}>
